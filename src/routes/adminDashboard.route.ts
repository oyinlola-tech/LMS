...
export default async function(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/export', { preHandler: [fastify.authenticate, fastify.requireRole(UserRole.ADMIN)] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const range = String((request.query as any).range || '7d');
      const rangeDays = range === '30d' ? 30 : 7;
      const now = new Date();
  ...
  fastify.get('/report', { preHandler: [fastify.authenticate, fastify.requireRole(UserRole.ADMIN)] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.get('/audit', { preHandler: [fastify.authenticate, fastify.requireRole(UserRole.ADMIN)] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
