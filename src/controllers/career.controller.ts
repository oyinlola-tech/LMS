...
export async function listActiveCareers(_request: FastifyRequest, reply: FastifyReply) {
  try {
...
export async function listAllCareers(_request: FastifyRequest, reply: FastifyReply) {
  try {
...
export async function getCareer(request: FastifyRequest, reply: FastifyReply) {
  try {
...
export async function createCareer(request: FastifyRequest, reply: FastifyReply) {
...
export async function updateCareer(request: FastifyRequest, reply: FastifyReply) {
  try {
    const { id } = request.params as { id: string };
    const career = await Career.findByPk(id);
    if (!career) return error(reply, 404, 'NOT_FOUND', 'Career not found');
...
export async function deleteCareer(request: FastifyRequest, reply: FastifyReply) {
  try {
    const { id } = request.params as { id: string };
    const career = await Career.findByPk(id);
    if (!career) return error(reply, 404, 'NOT_FOUND', 'Career not found');
