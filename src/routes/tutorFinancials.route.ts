...
export default async function (fastify: FastifyInstance): Promise<void> {
  fastify.get('/earnings', { preHandler: [fastify.authenticate, fastify.requireRole(UserRole.TUTOR)] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.get('/courses/analytics', { preHandler: [fastify.authenticate, fastify.requireRole(UserRole.TUTOR)] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const tutorId = request.user!.sub;
  ...
  fastify.post('/payouts/request', { preHandler: [fastify.authenticate, fastify.requireRole(UserRole.TUTOR)] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const tutorId = request.user!.sub;
  ...
  fastify.get('/payouts/history', { preHandler: [fastify.authenticate, fastify.requireRole(UserRole.TUTOR)] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const tutorId = request.user!.sub;
